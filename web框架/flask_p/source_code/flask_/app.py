...
from tag1_0_1.werkzeug.datastructures import Headers
from tag1_0_1.werkzeug.datastructures import ImmutableDict
from tag1_0_1.werkzeug.exceptions import BadRequest
from tag1_0_1.werkzeug.exceptions import BadRequestKeyError
from tag1_0_1.werkzeug.exceptions import default_exceptions
from tag1_0_1.werkzeug.exceptions import HTTPException
from tag1_0_1.werkzeug.exceptions import InternalServerError
from tag1_0_1.werkzeug.exceptions import MethodNotAllowed
from tag1_0_1.werkzeug.routing import BuildError
from tag1_0_1.werkzeug.routing import Map
from tag1_0_1.werkzeug.routing import RequestRedirect
from tag1_0_1.werkzeug.routing import RoutingException
from tag1_0_1.werkzeug.routing import Rule
from tag1_0_1.werkzeug.wrappers import BaseResponse
...
class Flask(_PackageBoundObject):
    ...
    def run(self, host=None, port=None, debug=None, load_dotenv=True, **options):
        ...
        from tag1_0_1.werkzeug.serving import run_simple
    ...
    def handle_exception(self, e):
        ...
        """
        exc_type, exc_value, tb = sys.exc_info()
        # 触发got_request_exception的信号(异常报错时触发)
    ...
    def full_dispatch_request(self):
        ...
        try:
            # 触发request_started的信号
            request_started.send(self)

            # 执行@app.before_request装饰的函数
        ...
        # 对返回值进行处理
        # 1. 处理＠app.after_request
        # 2. 进行session.save
        # 3. 触发request_finished信号
    ...
    def try_trigger_before_first_request_functions(self):
        ...
        with self._before_request_lock:
            if self._got_first_request:
                return
            # 循环列表执行@app.before_first_request 装饰的韩式
    ...
    def preprocess_request(self):
        ...
        for func in funcs:
            rv = func()
            # 当函数的返回不为空时则直接进行返回不继续执行下一个@app.before_request装饰的函数
            if rv is not None:
                return rv
    ...
    def request_context(self, environ):
        # request的上下文对RequestContext,进行压栈出栈使用.
    ...
    def wsgi_app(self, environ, start_response):
        # environ 为werkzeug得到的请求传递来的信息
        # start_response为werkzeug中的函数,用来返回flask处理完成的结果传递给werkzeug
        ...
        :param start_response: A callable accepting a status code,
            ...
            start the response.
        """
        #  ctx为一个RequestContext对象．
        # ctx.app
        # ctx.requset
        # ctx.session 都存在
        # 将请求的相关数据environ封装到RequestContext对象中去
        ...
        try:
            try:
                ...
                将对象RequestContext(ctx)封装到local中(每个线程，每个协程独立空间存储)
                获取到唯一标示
                _request_ctx_stack =
                {
                 唯一id:{
                      stack:[requestContext对象(包含request和session)]
                      }
                }
                push 后ctx.session中已经获取到了值
                
                
                
                app_ctx = AppContext 对象
                app_ctx.app 当前app对象
                app_ctx.g 对象(用于保存一个周期中药存储的值)
                ...
                获取到session,并保存到RequestContext中
                """
                #
                ctx.push()
            # 寻找视图函数并执行
            # 获取返回值
                # 1. 首先执行@app.before_first_request的函数
                # 2. 触发request_started的信号
                # 3. 执行@app.before_request装饰的函数
                # 4. 执行视图函数
                #  a.触发before_render_template的信号
                #  b. 进行模板渲染
                #  c. 触发template_rendered的信号 
                # 5. 处理＠app.after_request
                # 6. 进行session.save
            ...
            if self.should_ignore_error(error):
                error = None
            # 执行@app.teardown_request装饰的函数 
            # 触发request_tearing_down信号
            # 最后将请求在local中保存的RequestContext对象进行清除
            ctx.auto_pop(error)
